	...
	switch ($nirvana_siteheader) {
		...
		case 'Custom Logo' :
			if (isset($nirvana_logoupload) && ($nirvana_logoupload != '')) :
				echo '<div class="site-identity"><a id="logo" href="'.esc_url( home_url( '/' ) ).'" ><img title="'.esc_attr( get_bloginfo( 'name', 'display' ) ).'" alt="'.esc_attr( get_bloginfo( 'name', 'display' ) ).'" src="'.$nirvana_logoupload.'" /></a></div>';
			endif;
		break;
...
function cryout_search_footermenu( $items, $args ) { 
	...
	if( $args->theme_location == 'footer') {
		$items = $items."<li class='menu-footer-search'>
							<form action='".esc_url(home_url( '/' ))."' class='searchform' method='get'>
								<input type='text' name='s' class='s' placeholder='".__('Search','nirvana')."...'>
